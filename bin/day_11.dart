...
void main() async {
  ...
  final deviceMap = {
    for (var parts in lines.map((line) => line.split(': ')))
...
typedef Search = ({String device, bool dac, bool fft});
...
Future<void> part2(Devices deviceMap) async {
  ...
  int countPaths(Search search) {
    ...
    if (search.device == 'out') {
      return search.dac && search.fft ? 1 : 0;
    }
    final dev = deviceMap[search.device];
    if (dev == null) {
      return 0;
    }
    ...
    final sum = dev
        .map(
          (d) => countPaths((
            device: d,
            dac: search.dac || search.device == 'dac',
            fft: search.fft || search.device == 'fft',
          )),
        )
        .sum;
    cache[search] = sum;
  ...
  print(countPaths((device: 'svr', dac: false, fft: false)));
}
